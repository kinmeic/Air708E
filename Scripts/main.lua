...
sys.taskInit(function()
    ...
    table.insert(buff,{SERIALNUM, "#BOOT"})
    sys.publish("SMS_ADD")--推个事件
    ...
    while true do
        ...
        while #buff > 0 do--把消息读完
            ...
            if useServerChan then--server酱
                ...
                log.info("notify","send to self push server",data)

                local num = mobile.number(0)
                local msg = ""

                msg = msg .. "【短信转发-" .. SERIALNUM .. "】\n"
                msg = msg .. data .. "\n"
                msg = msg .. "发件人：" .. sms[1] .. "\n"
                msg = msg .. "收件人：" .. num
